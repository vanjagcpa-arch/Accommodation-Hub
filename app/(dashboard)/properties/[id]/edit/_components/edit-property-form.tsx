...
import { updateProperty, type ActionState } from '@/lib/properties/actions'
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <Select id="building_id" name="building_id" defaultValue={property.building_id} required>
              {buildings.map(b => (
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Input id="unit_number" name="unit_number" defaultValue={property.unit_number} required />
            </div>
            <div>
              <Label htmlFor="property_type">Property Type</Label>
              <Select id="property_type" name="property_type" defaultValue={property.property_type ?? ''} placeholder="Select type">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <Input id="bedrooms" name="bedrooms" type="number" min="0" max="10" defaultValue={property.bedrooms} />
            </div>
            <div>
              <Label htmlFor="bathrooms">Bathrooms</Label>
              <Input id="bathrooms" name="bathrooms" type="number" min="0.5" max="5" step="0.5" defaultValue={property.bathrooms} />
            </div>
            <div>
              <Label htmlFor="floor_level">Floor Level</Label>
              <Input id="floor_level" name="floor_level" type="number" min="-2" max="100" defaultValue={property.floor_level ?? ''} placeholder="0 = ground" />
            </div>
          </div>

          <div>
            <Label htmlFor="size_sqm">Size (m²)</Label>
            <Input id="size_sqm" name="size_sqm" type="number" step="0.1" defaultValue={property.size_sqm ?? ''} />
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Input id="rent_amount" name="rent_amount" type="number" step="0.01" defaultValue={property.rent_amount ?? ''} />
            </div>
            <div>
              <Label htmlFor="bond_amount">Bond Amount (AUD)</Label>
              <Input id="bond_amount" name="bond_amount" type="number" step="0.01" defaultValue={property.bond_amount ?? ''} />
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input id="available_date" name="available_date" type="date" defaultValue={property.available_date ?? ''} />
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 p-3 bg-surface-muted rounded-lg border border-line">
            <input
              ...
              id="agent_visible"
              name="agent_visible"
              defaultChecked={property.agent_visible}
            ...
            {ALL_FEATURES.map((feature) => (
              <label key={feature} className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  name="features"
                  value={feature}
                  defaultChecked={property.features?.includes(feature) ?? false}
                  className="w-4 h-4 rounded border-line text-primary focus:ring-primary"
                />
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Input id="reapit_external_id" name="reapit_external_id" defaultValue={property.reapit_external_id ?? ''} />
            </div>
            <div>
              <Label htmlFor="listonce_external_id">ListOnce ID</Label>
              <Input id="listonce_external_id" name="listonce_external_id" defaultValue={property.listonce_external_id ?? ''} />
            </div>
          </div>
          <div>
            <Label htmlFor="ezidebit_code">Ezidebit Code</Label>
            <Input id="ezidebit_code" name="ezidebit_code" defaultValue={property.ezidebit_code ?? ''} />
          </div>
        </CardContent>
      </Card>
