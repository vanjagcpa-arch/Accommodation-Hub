...
import { createMaintenanceJob, type ActionState } from '@/lib/maintenance/actions'
  ...
  return (
    <form action={formAction} className="space-y-5">
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <textarea id="description" name="description" rows={3}
              placeholder="Describe the issue, location and any symptoms…"
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select id="tenant_id" name="tenant_id" className={selectClass} defaultValue="">
            ...
            <div>
              ...
              <Input id="reported_by_name" name="reported_by_name" placeholder="e.g. Tenant, manager, inspection" />
            </div>
          </div>
          <div>
            <Label htmlFor="preferred_access_window">Preferred access</Label>
            <Input id="preferred_access_window" name="preferred_access_window" placeholder="e.g. Weekdays 9am–4pm" />
          </div>
          <div>
            <Label htmlFor="access_notes">Access notes</Label>
            <textarea id="access_notes" name="access_notes" rows={2}
              placeholder="Key location, lockbox code, pets, tenant availability…"
              className="w-full rounded-lg border border-line-strong bg-surface px-3 py-2 text-sm text-ink placeholder:text-ink-subtle focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary-ring/50 resize-none" />
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select id="assigned_staff_id" name="assigned_staff_id" className={selectClass} defaultValue="">
                <option value="">Leave unassigned</option>
                {options.staff.map((s) => <option key={s.id} value={s.id}>{s.full_name}{s.trade ? ` · ${s.trade}` : ''}</option>)}
              </select>
            </div>
            <div>
          ...
          <div>
            ...
            <textarea id="internal_notes" name="internal_notes" rows={2}
              placeholder="Not visible to tenants or referral agents…"
              className="w-full rounded-lg border border-line-strong bg-surface px-3 py-2 text-sm text-ink placeholder:text-ink-subtle focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary-ring/50 resize-none" />
          </div>
      ...
      <div className="flex items-center justify-end gap-3">
        <a href="/maintenance" className="text-[13px] text-ink-muted hover:text-ink">Cancel</a>
        <Button type="submit" loading={pending}>Create Job</Button>
      </div>
    </form>
  )
}
