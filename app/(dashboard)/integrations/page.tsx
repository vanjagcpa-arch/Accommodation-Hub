...
const integrations = [
  ...
  },
  {
    ...
    note: 'External ID mapping is configured — tenant codes are stored for all eligible properties. Connect your Ezidebit credentials to enable automated charge uploads.',
  },
]
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="space-y-6">
        {integrations.map((integration) => {
          const statusConf = statusConfig[integration.status as keyof typeof statusConfig]
          return (
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { system: 'Reapit', entities: 'Properties, Buildings, Applications, Tenants', ids: '159 mapped' },
              { system: 'ListOnce', entities: 'Properties, Buildings', ids: '24 mapped' },
              { system: 'MYOB', entities: 'Companies', ids: '1 mapped' },
              { system: 'Ezidebit', entities: 'Properties, Tenants', ids: '17 mapped' },
            ].map(item => (
              <div key={item.system} className="p-3 bg-slate-50 border border-slate-200 rounded-lg">
                ...
                <p className="text-xs text-green-600 font-medium mt-1">{item.ids}</p>
              </div>
            ))}
          </div>
        </CardContent>
