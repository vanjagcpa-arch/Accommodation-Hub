...
export default function ContractorsClient({ contractors, error, buildings = [] }: Props) {
          ...
          <div className="rounded-xl border border-line bg-surface overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-canvas border-b border-line">
                <tr>
                  {['Name', 'Trade', 'Email', 'Phone', 'Rate/hr', 'Callout'].map(h => (
              ...
              <tbody className="divide-y divide-line">
                {filtered.map(c => (
                  <tr key={c.id} className="hover:bg-canvas/50">
                    ...
                    <td className="px-4 py-3 font-mono text-ink-muted text-xs">
                      {c.callout_fee != null ? `$${c.callout_fee}` : '—'}
                    </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-2xl bg-surface shadow-xl">
            ...
            <form onSubmit={handleAdd} className="p-6 space-y-4">
              ...
              {buildings.length > 0 && (
                <div>
                  <label className="block text-sm font-medium text-ink mb-1">Home base building</label>
                  <select name="home_base_building_id" className="w-full rounded-lg border border-line px-3 py-2 text-sm text-ink focus:outline-none focus:ring-2 focus:ring-primary/50">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-medium text-ink mb-1">Hourly rate ($)</label>
                  <input name="hourly_rate" type="number" min="0" step="0.01" placeholder="e.g. 85" className="w-full rounded-lg border border-line px-3 py-2 text-sm text-ink focus:outline-none focus:ring-2 focus:ring-primary/50" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-ink mb-1">Callout fee ($)</label>
                  <input name="callout_fee" type="number" min="0" step="0.01" placeholder="e.g. 120" className="w-full rounded-lg border border-line px-3 py-2 text-sm text-ink focus:outline-none focus:ring-2 focus:ring-primary/50" />
                </div>
              </div>
